...
/**
 * Resolve created_at for logins. Lookup failure → { ok: false } (unknown, not "young").
 * Use as tallyVotes getCreatedAt.
 */
export async function resolveUserCreatedAts(logins) {
  const map = new Map();
  for (const login of logins) {
    if (!login || map.has(login)) continue;
    try {
      const u = await getUser(login);
      map.set(login, { ok: true, createdAt: u?.created_at || null });
    } catch {
      map.set(login, { ok: false, createdAt: null });
    }
  }
  return map;
}
...
export async function listIssues({ state = "all", creator = null, since = null, maxPages = 5 } = {}) {
  const all = [];
  const sinceMs = since ? Date.parse(since) : null;
  for (let page = 1; page <= maxPages; page++) {
    const params = new URLSearchParams({ state, per_page: "100", page: String(page) });
    if (creator) params.set("creator", creator);
    const batch = await gh(`/issues?${params}`);
    const items = batch.filter((i) => !i.pull_request);
