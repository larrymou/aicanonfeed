...
export async function listIssueReactions(issue_number) {
  return gh(`/issues/${issue_number}/reactions?per_page=100`);
}
...
export async function fileExistsOnRef(filePath, ref = "main") {
  try {
    await gh(`/contents/${filePath}?ref=${encodeURIComponent(ref)}`);
    return true;
  } catch {
    return false;
  }
...
/** Create/update a single file on a branch and open a PR if needed. */
